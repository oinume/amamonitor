...
// GooseDbVersion represents a row from 'amamonitor.goose_db_version'.
...
func (gdv *GooseDbVersion) Insert(db XODB) error {
	...
	const sqlstr = `INSERT INTO amamonitor.goose_db_version (` +
...
func (gdv *GooseDbVersion) Update(db XODB) error {
	...
	// sql query
	const sqlstr = `UPDATE amamonitor.goose_db_version SET ` +
		`version_id = ?, is_applied = ?, tstamp = ?` +
		` WHERE id = ?`
...
func (gdv *GooseDbVersion) Delete(db XODB) error {
	...
	// sql query
	const sqlstr = `DELETE FROM amamonitor.goose_db_version WHERE id = ?`
